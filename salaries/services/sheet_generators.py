...
def generate_tinkoff_payment_sheet_by_salary_reports(reports_queryset) -> io.BytesIO:
    target_output = io.BytesIO()
    workbook = xlsxwriter.Workbook(target_output, {"in_memory": True})
    ...
    for i, result in enumerate(results_map.items()):
        ...
        expenses.append(
            (
                ...
                result[1],
                employee.default_payment_details,
                1,
                employee.bank_id,
            )
        )
