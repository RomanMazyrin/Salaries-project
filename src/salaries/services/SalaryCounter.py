...
class SalaryCounter:
    ...
    def __get_metrics_for_calls(self, timestamp_from, timestamp_to):
        ...
        if self.__employee.onpbx_id:
            ...
            while True:
                ...
                onpbx_calls_outbound.extend(
                    self.__onpbx_client.call_history.get(
        ...
        sipuni_outbound_calls_count = len(
            list(
                filter(
    ...
    def __get_metrics_for_sales(self, timestamp_from, timestamp_to):
        ...
        if self.__employee.sale_fee_percent:
            ...
            metrics.extend(
                self.get_metrics_from_leads(leads, 1212574, "licenses", "лицензий", "лицензии")
            )
            metrics.extend(
                self.get_metrics_from_leads(
            ...
            metrics.extend(
                self.get_metrics_from_leads(leads, 3941655, "courses", "Курсов", "курсы")
            )

        if self.__employee.one_hour_salary_amount:
            hours_list = [
                self.__find_custom_field_value_in_lead(lead, 682570)
                for (lead, payment) in leads
                if lead["pipeline_id"] in [3346951]
            ]
        ...
        if self.__employee.one_feedback_cost:
            ...
            feedback_count = [
                self.__find_custom_field_value_in_lead(lead, 683742)
                for (lead, payment) in leads
                if lead["pipeline_id"] in [3964107, 3678177]
            ]
                ...
                "filter": json.dumps(
                    {
                        ...
                        "custom_fields": {"489154": {"from": timestamp_from, "to": timestamp_to}},
                    }
                ),
            },
            entity_type="leads",
    ...
    def __get_email_outcome_messages_count_in_period(self, timestamp_from, timestamp_to):
        ...
        mail.login(self.__employee.email_imap_login, self.__employee.email_imap_password)
        ...
        result, data = mail.search(
            None,
            '(since "{}" before "{}")'.format(
                date_from.strftime(date_format), date_to.strftime(date_format)
            ),
        )
    ...
    def __get_metrics_for_license_activations(self, timestamp_from, timestamp_to):
        ...
        leads = self.__fetch_all_entities(
            ...
            params={
                ...
                "filter": json.dumps(
                    {
                        "closed_at": {"from": timestamp_from, "to": timestamp_to},
                        "statuses": [{"status_id": 142, "pipeline_id": 1212574}],
                    }
                ),
            },
            entity_type="leads",
    ...
    def get_detailed_report(self, timestamp_from, timestamp_to):
        ...
        metrics_fetchers = [
            self.__get_metrics_for_calls,
            self.__get_metrics_for_salary,
            self.__get_amocrm_metrics,
            self.__get_metrics_for_outcome_email_messages,
            self.__get_metrics_for_license_activations,
        ]

        metrics_results = [req(timestamp_from, timestamp_to) for req in metrics_fetchers]
        ...
        money_amount = sum(
            [
                metrica.value
                for metrica in report.get_metrics_by_meta_param(
    ...
    def get_metrics_from_leads(
        self, leads_list, pipeline_id_list, group, entity_name, entity_name_plural
    ):
        ...
        return [
            Metrica(
                ...
                len(
                    [
                        lead
                        for (lead, payment) in leads_list
                        if lead["pipeline_id"] in pipeline_id_list
                    ]
                ),
                group,
            ),
            Metrica(
                entity_name + " продано на сумму",
                sum(
                    [
                        lead["price"]
                        for (lead, payment) in leads_list
            ...
            Metrica(
                ...
                math.floor(
                    sum(
                        [
                            payment_amount
                            for (lead, payment_amount) in leads_list
                            if lead["pipeline_id"] in pipeline_id_list
                        ]
                    )
                ),
                ...
                "filter": json.dumps(
                    {
                        ...
                        "statuses": [
                            ...
                            {
                                "status_id": 142,
                                "pipeline_id": self.COLD_CALLS_PIPELINE_ID,
                            },
                        ],
                    }
                ),
